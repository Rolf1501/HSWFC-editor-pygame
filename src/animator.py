...
class Animator(ShowBase):
    ...
    def scale_to_unit(self, model: NodePath) -> Coord:
        ...
        scaled = Coord(
            self.unit_dims.x / (1.0 * model_extent.x),
            self.unit_dims.y / (1.0 * model_extent.y),
            self.unit_dims.z / (1.0 * model_extent.z),
        )
        return scaled
...
class GSAnimator(Animator):
    ...
    def add_parts(self):
        for p in self.parts.values():
            ...
            self.make_model(
                p_origin, p_extent, colour=p.colour, compensate_extent=False
            )
            print(f"Part: {p}")
