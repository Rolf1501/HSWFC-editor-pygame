...
class Terminal:
    ...
    distinct_orientations: list[int] = field(default=[0])
    ...
    oriented_atom_mask: dict[int, np.ndarray] = field(init=False)
    oriented_indices: dict[int, np.ndarray] = field(init=False)
    ...
    def calc_oriented_masks(self):
        ...
        self.n_atoms = 0
        for o in self.distinct_orientations:
            # The orientation determines how many counter clockwise rotations of 90 degrees are needed.
            self.oriented_mask[o] = np.rot90(self.mask, o, axes=(0, 1))
            indices = self.calc_atom_indices(o, self.oriented_mask[o])
            self.oriented_indices[o] = indices

            self.oriented_atom_mask[o] = np.full(
            ...
            for i in range(len(indices)):
                c = indices[i]
                curr = self.oriented_atom_mask[o][c.y, c.x, c.z]
                curr[i] = True
        ...
        for o in self.distinct_orientations:
            for axis in axes.keys():
                ...
                hm_along_orr, hm_against_orr = self.calc_heightmap_for_axis(
                    self.oriented_mask[o], axis=axis
                )
                self.oriented_heightmaps[o][cardinal_along] = hm_along_orr
                self.oriented_heightmaps[o][cardinal_against] = hm_against_orr
